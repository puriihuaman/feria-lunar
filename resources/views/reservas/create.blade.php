...
<div class="container">
  <h2>Reservar Stand</h2>

  <div class="card mb-4">
      <div class="card-body">
          <h4>{{ $sedeStand->sede->title }}</h4>
          <p><strong>Dirección:</strong> {{ $sedeStand->sede->address }}</p>
          <p><strong>Stand:</strong> #{{ $sedeStand->stand->booth_number }} ({{ ucfirst($sedeStand->stand->category) }})</p>
          <p><strong>Precio:</strong> S/. {{ number_format($sedeStand->price, 2) }}</p>
          <p><strong>Fecha de reserva:</strong> {{ \Carbon\Carbon::parse($selectedDate)->format('d M Y') }}</p>
      </div>
  </div>
